...
const Detail = ({exerciseDetail}) => {
  ...
  return (
            ...
            </Typography>
            {
                extraDetail.map((item)=> (
                    <Stack key={item.index} direction="row" gap="24px" alignItems="center">
                        ...
                        <Typography  textTransform="capitalize" sx={{ fontSize: { lg: '30px', xs: '20px' },color:"var(--color2)" }}>
                            {item.name}
                        </Typography>
                    </Stack>
                ))
    ...
    <Typography variant='h2'textTransform="capitalize" alignSelf='self-start' id="instructions" fontWeight={700}
    sx={{color:"var(--color2)"}}
    >
            Instructions...
    ...
    <Stack mt="5px" gap={1.3} alignSelf='self-start'  >
                {
                    instructions?.map((instruction)=>(
                        <Typography key={instruction.index} variant='h5' fontWeight={400}
                        sx={{color:"var(--color1)"}}  >
                            {instruction}
                        </Typography>
                    ))
